...
# Set both gamma rate and epsilon rate to a binary string/list of the same length as the inputs
number_length = len(numbers[0])
gamma_rate = ["0"] * number_length
epsilon_rate = ["0"] * number_length

for pos in range(0, number_length):
    ones = 0
    zeros = 0
    for n in numbers:
        if n[pos] == "0":
...
# print("Gamma: " + str(gamma_rate) + " Epsilon: " + str(epsilon_rate))
gamma_int = int("".join(g for g in gamma_rate), 2)
epsilon_int = int("".join(e for e in epsilon_rate), 2)
